...
header = [
    ...
    text "",
    text "@SuppressWarnings" <> text "(" <> text "{" <> string "unused" <> text "," <+> string "unchecked" <> text "}" <> text ")"
 ]
...
mkSerializer ty@(AType nm tv) cons = vcat [
    ...
    block $ concat [
        [
             ...
             nest 4 $ mkSingleSerializer ty' con <> text ";"
        ]
        | con <- cons, let nm' = consName con, let ty' = AType nm' tv
         ...
         block $ concat [
             [
                 ...
                 nest 4 $ text "return" <+> ident [nm', "serializer"] <> text "." <> text "serialize" <> text "(" <> text "val" <> text "." <> ident ["get", nm'] <> text "()" <> text ")" <> text ";"
             ]
