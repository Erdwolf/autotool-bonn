...
($\alpha$, $\beta$) & Paar von Typ $\alpha$ und $\beta$\\
...
\begin{itemize}
\item Bei inkompatiblen Änderungen soll die major-Version erhöht werden.
\item Bei abwärtskompatiblen Änderungen soll die minor-Version erhöht werden.
\item Bei kompatiblen Änderungen soll die micro-Version erhöht werden.
\end{itemize}
...
Daneben wird angegeben, in welcher Reihenfolge Punkte für Toplisten ausgewertet werden sollen. Siehe \texttt{ScoringOrder} (\ref{ScoringOrder}).
...
\begin{itemize}
\item \texttt{Decreasing} -- absteigende Sortierung. Das heißt, eine größere Größe ist besser als eine kleinere.
\item \texttt{Increasing} -- aufsteigende Sortierung. Kleinere Größen sind besser.
\item \texttt{None} -- keine Angabe, z.B. weil die Sortierreihenfolge von der Aufgabenstellung abhängt, oder weil die berechnete Größe nicht variiert.
\end{itemize}
