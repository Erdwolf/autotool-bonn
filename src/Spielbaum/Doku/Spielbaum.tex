...
\item Weiterhin soll die Anbindung an den Spiele-Server erreicht
  werden, damit das Spielen gegen einen Gegner ermöglicht wird
  (Projekt bereits von anderer Gruppe in Arbeit soweit ich weiß).
...
kom\-bi\-na\-to\-rischen Logik mit Mustervergleich aufgefasst werden. Da in
...
Gegeben sei eine Menge von Variablen \underline{Var} und eine Menge von
Konstanten \underline{Const}. Dann ist die Menge der Terme
definiert durch:
\begin{quote}
...
\item Das TRS heißt \underline{partiell mehrdeutig} gdw. es existieren
  partiell über\-lap\-pen\-de Regeln.
\end{itemize}
\bigskip
{\sc Nichtlinearität:} 
\begin{itemize}
...
\item Ein TRS heißt \underline{konfluent} (Church-Rosser-Eigenschaft),
  ...
  t_3$ und $t_2 \Rightarrow^* t_3$. 
\item Ein TRS heißt \underline{orthogonal} gdw. das TRS eindeutig
  (d.h. nicht mehr\-deu\-tig) und linkslinear ist.
\end{itemize}
...
und be\-steht daher aus einem String, der die aktuelle Spielposition
...
{\tt $\sim$/autotool/autobahn/Original.hs}), und zwar folgendermaßen: Ich nehme
...
Vor\-gehens\-weise: In einem Schritt werden alle in einem Schritt
...
lassen, aber um ein paar Beispiele ordentlich graphisch zu
veranschaulichen, ist das Programm ausreichend. Die Graphen sind sachlich
...
{\tt 
\begin{tabular}[h]{ll}
instanzTrans :: ShowText knoten => & (knoten -> Int) \\
& -> GVTrans knoten \\
\end{tabular}
}
\begin{itemize}
...
{\tt 
\begin{tabular}[h]{ll}
renameNodes ::&[ ( String, [String] ) ] -> [ SpielbaumLabel ] \\
 &-> [ SpielbaumLabel ] \\
\end{tabular}
}
\begin{itemize}
...
\begin{itemize}
\item Parameter: Wort, mit dem gespielt wird, d.h. Startwort und
  zugehörige Regeln
\end{itemize}
\begin{enumerate}
...
\end{itemize}
\begin{enumerate}
\item alle Escape-Sequenzen werden durch ShowText des Graph.Viz
...
aus\-führ\-lich genug unter Anwendungsbeispiele (Punkt 2.4.) behandelt.
...
damit erzeug\-ten Graphen sind inhaltlich richtig und können wunschgemäß
...
erreichte her\-vor\-zu\-heben. Dafür eignet sich die graphische Darstellung
...
Spiel\-baum-Modul im Hugs startet. Am einfachsten funktioniert das ganze
Im\-por\-tier\-en, wenn man im Verzeichnis {\tt autotool} mit dem
Aufruf

{\tt hugs -98 +o -Putil: Spielbaum/Main.hs} 
...
``hübsche'' Prelude-Funk\-tionen wie map und foldr kennt und verstanden
