...
instance Generator EditDistance QuizConfig Config where
    generator p (QuizConfig fb n m a e) key = do -- IO
      uncurry (Config fb) <$> EditDistance.Generator.generate n m a
...
make :: Make
make = quiz EditDistance (QuizConfig NumberOfErrors 5 7 4 0)
