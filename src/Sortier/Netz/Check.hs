...
check soll n = do
    ...
    inform $ text "Das Netz hat alle m√∂glichen Eingaben korrekt geordnet."
    return ()

-- | es gilt ja der satz: 
-- alle 0-1-folgen sortiert <=> ¸berhaupt alle folgen sortiert.
-- also generiere ich 0-1-folgen (weil das weniger sind)
-- aber um die studenten nicht unnˆtig aufzuregen,
...
instance C.Partial Sortier Int Netz where
    ...
    describe p i = vcat 
	  [ text "Finden Sie ein Sortiernetz f√ºr"
	         <+> toDoc i <+> text "Eing√§nge"
