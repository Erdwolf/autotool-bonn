...
module Codec.Encryption.MD5Aux 
   (md5,  md5s,  md5i,
    MD5(..), ABCD(..), 
...
instance Num ABCD where
   ABCD (a1, b1, c1, d1) + ABCD (a2, b2, c2, d2) 
       = ABCD (a1 + a2, b1 + b2, c1 + c2, d1 + d2)
   (*) = error "Codec.Encryption.MD5Aux does not implement Num.* for ABCD"
   signum = error "Codec.Encryption.MD5Aux does not implement Num.signum for ABCD"
   abs = error "Codec.Encryption.MD5Aux does not implement Num.abs for ABCD"
   fromInteger = error "Codec.Encryption.MD5Aux does not implement Num.fromInteger for ABCD"
