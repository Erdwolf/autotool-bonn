...
import Knapsack.Solve ( packs )
import Knapsack.Param
...
data Knapsack = Knapsack deriving ( Eq, Typeable, Show, Read )

fm_fun :: FiniteMap Char a -> Char -> a
fm_fun fm = lookupWithDefaultFM fm (error "data incomplete!?")

count :: (Char -> Integer) -> FiniteMap Char Rational -> Rational
count f = foldFM ( \ x r v -> v + r * (f x % 1) ) 0

type Pack = (Rational,FiniteMap Char Rational)

instance Size Pack where size = const 1

instance Partial Knapsack Inp Pack where

    describe Knapsack inp = vcat
    ...
    partial Knapsack inp (value,packFM) = do
    ...
    total Knapsack inp (value,packFM) = do
...
make_fixed :: Make
make_fixed = direct Knapsack inp0

instance Generator Knapsack Param ( Inp , Pack ) where
    generator _ conf _ = do

        let os = take (anzahl conf) ['a'..]
...
instance Project Knapsack (Inp,Pack) Inp where project _ = fst

make_quiz :: Make
make_quiz = quiz Knapsack p0
