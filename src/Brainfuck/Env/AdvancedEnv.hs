...
module Brainfuck.Env.AdvancedEnv where 
...
instance ToDoc [ Statement ] where
    toDoc sts = braces ( text "" <+> ( hcat ( map toDoc sts )) <+> text "")
...
dec :: Env -> Env
dec (Env l (x : xs) i o c s p)  = Env {left = l, right = [(max (x-1) 0)] ++ xs, input = i, output = o, cmd = tail c, steps = s+1, previous = [ (Env l (x:xs) i o c s p) ] ++ p}


shift_r :: Env -> Env
shift_r (Env l (x : xs) i o c s p) = case xs of
                          [] -> if l == [] && x == 0
                                then Env {left = [], right = [0], input = i, output = o, cmd = tail c, steps = s+1, previous = [ (Env l (x:xs) i o c s p) ] ++ p}
                                else Env {left = [x] ++ l, right = [0], input = i, output = o, cmd = tail c, steps = s+1, previous = [ (Env l (x:xs) i o c s p) ] ++ p}
                          _  -> if l == [] && x == 0 
                                then Env {left = [], right = xs, input = i, output = o, cmd = tail c, steps = s+1, previous = [ (Env l (x:xs) i o c s p) ] ++ p}
                                else Env {left = [x] ++ l, right = xs, input = i, output = o, cmd = tail c, steps = s+1, previous = [ (Env l (x:xs) i o c s p) ] ++ p}


shift_l :: Env -> Env
shift_l (Env l (rx : rxs) i o c s p) = case l of
                  [] -> if rx == 0 && rxs == []
                        then Env {left = [], right = [0], input = i, output = o, cmd = tail c, steps = s+1, previous = [ (Env l (rx:rxs) i o c s p) ] ++ p}
                        else Env {left = [], right = [0] ++ [rx] ++ rxs, input = i, output = o, cmd = tail c, steps = s+1, previous = [ (Env l (rx:rxs) i o c s p) ] ++ p}
                  (lx : lxs) -> if rx == 0 && rxs == []
                                then Env {left = lxs, right = [lx], input = i, output = o, cmd = tail c, steps = s+1, previous = [ (Env (lx:lxs) (rx:rxs) i o c s p) ] ++ p}
                                else Env {left = lxs, right = [lx] ++ [rx] ++ rxs, input = i, output = o, cmd = tail c, steps = s+1, previous = [ (Env (lx:lxs) (rx:rxs) i o c s p) ] ++ p}
