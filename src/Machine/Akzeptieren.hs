...
positiv_liste :: ( Machine m dat conf, ToDoc [dat] )
	      => Int -> m -> [dat]
	      -> Reporter ()
positiv_liste cut m  xss = check_liste True cut m xss

negativ_liste :: ( Machine m dat conf, ToDoc [dat] )
	      => Int -> m -> [dat] 
              -> Reporter ()
negativ_liste cut m xss = check_liste False cut m xss
