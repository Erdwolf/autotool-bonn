...
instance C.Partial Program_Array ( Program, Environment ) Environment where
    ...
    initial _ (p, e) = Program.ArrayBonn.Environment.example

    total _ ( p , target) start = do
        inform $ text "Ich führe das Programm aus:"
        actual <- nested 4 $ Program.ArrayBonn.Semantics.execute start p
	inform $ vcat
...
make_fixed = direct 
       Program_Array	
       ( Program.ArrayBonn.Program.s , Program.Array.Environment.example )
